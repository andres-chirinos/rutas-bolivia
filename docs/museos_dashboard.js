...
(async () => {
    ...
    const cache = {
        ...
        getEntitySearch(entityId, radius, includeMuseums, includeTransport) {
            const key = this.getEntitySearchKey(entityId, radius, includeMuseums, includeTransport);
    ...
    function addToTour(id, label, coordinates) {
        ...
        if (popup && popup.isOpen()) {
            ...
            const buttonText = isNowInTour ? '➖ Quitar de la ruta' : '➕ Añadir a la ruta';
    ...
    function toggleTourMode() {
        ...
        info.textContent = 'Modo Tour activo. Haz clic en los museos para ver su información y añadirlos a la ruta.';
            ...
            if (route && route.summary) {
                ...
                if (route.steps && route.steps.length > 0) {
                    ...
                    route.steps.forEach((step, stepIndex) => {
                        const stepIcon = step.type === 'walk' ? '🚶' : step.type === 'transfer' ? '🔄' : '🚌';
                        detailsHTML += `
    ...
    function renderEntitySearchResults(searchData) {
        ...
        if (entity && entity.coordinates) {
            ...
            // Marcador del centro
            const centerMarker = L.circleMarker([entity.coordinates[1], entity.coordinates[0]], {
                radius: 12,
                fillColor: '#FF5722',
                color: '#D84315',
                weight: 3,
            ...
            const searchCircle = L.circle([entity.coordinates[1], entity.coordinates[0]], {
            ...
            layerManager.addLayer('search_center', centerLayer, `Centro: ${entity.name}`);
        }
        ...
        if (museums && museums.features && museums.features.length > 0) {
            const museumsLayer = L.geoJSON(museums, {
                ...
                onEachFeature: function(feature, layer) {
                    ...
                    const buttonText = isInTour ? '➖ Quitar de la ruta' : '➕ Añadir a la ruta';
            ...
            layerManager.addLayer('search_museums', museumsLayer, `Museos (${museums.features.length})`);
        }
        
        // Mostrar redes de transporte encontradas
        transportNetworks.forEach((network, index) => {
            if (network.data && network.data.features && network.data.features.length > 0) {
                const networkLayer = L.geoJSON(network.data, {
                    style: function(feature) {
                        const colors = ['#2196F3', '#FF9800', '#9C27B0', '#E91E63', '#795548'];
                        return {
                ...
                layerManager.addLayer(`search_transport_${index}`, networkLayer, `${network.name} (${network.feature_count})`);
            }
        });
    ...
    function showEntitySearchSummary(searchData) {
        ...
        const existingSearchInfo = layerControls.querySelectorAll('div[data-search-summary="true"]');
    ...
    async function searchKnowledgeGraph() {
        ...
        if (query.length < 2) {
            info.textContent = 'El término de búsqueda debe tener al menos 2 caracteres.';
            return;
        }
        
        info.textContent = `Buscando "${query}" en el knowledge graph...`;
        kgSearchResults.innerHTML = '<div style="padding:10px; text-align:center;">🔍 Buscando...</div>';
        ...
        try {
            ...
            if (searchData) {
                ...
                const response = await fetch(searchUrl);
                if (!response.ok) {
                    throw new Error(`Error en búsqueda KG: ${response.status} ${response.statusText}`);
                }
    ...
    function renderKgSearchResults(searchData) {
        ...
        if (results.length === 0) {
            kgSearchResults.innerHTML = '<div style="padding:10px; color:#666;">🔍 No se encontraron resultados.</div>';
    ...
    async function showEntityDetails(entityId) {
        ...
        try {
            ...
            if (entityData) {
                ...
                if (!response.ok) {
                    throw new Error(`Error cargando entidad: ${response.status} ${response.statusText}`);
                }
    ...
    function renderEntityDetails(entityData) {
        ...
        const existingEntityInfo = layerControls.querySelectorAll('div[data-entity-details="true"]');
    ...
    async function searchExternalEntities() {
        ...
        externalResults.innerHTML = '<div style="padding:10px; text-align:center;">🔍 Buscando en KG externo...</div>';
        ...
        try {
            ...
            const response = await fetch(searchUrl);
            if (!response.ok) {
                throw new Error(`Error en búsqueda externa: ${response.status} ${response.statusText}`);
            }
    ...
    function renderExternalResults(searchData) {
        ...
        if (results.length === 0) {
            externalResults.innerHTML = '<div style="padding:10px; color:#666;">🔍 No se encontraron entidades externas.</div>';
    ...
    async function analyzeDataset() {
        ...
        analysisResults.innerHTML = '<div style="padding:10px; text-align:center;">📊 Analizando estructura de datos...</div>';
    ...
    function renderAnalysisResults(analysisData) {
        ...
        if (columns.length === 0) {
            analysisResults.innerHTML = '<div style="padding:10px; color:#666;">📄 No se pudieron extraer columnas del dataset.</div>';
            return;
        }
            ...
            resultsHtml += `
                <div style="margin-bottom:12px; padding:12px; border:1px solid #ddd; border-radius:6px; background:#fafafa;">
                    ...
                    <div style="margin-top:8px;">
                        ...
                        <div style="margin-top:4px;">
                            ${semanticMatches.slice(0, 3).map(match => `
    ...
    const layerManager = {
        layers: {
            ...
            search_center: { layer: null, name: 'Centro de búsqueda', visible: true, zIndex: 9, color: '#FF5722' },
            search_museums: { layer: null, name: 'Museos encontrados', visible: true, zIndex: 10, color: '#4CAF50' },
            search_transport: { layer: null, name: 'Transporte encontrado', visible: true, zIndex: 11, color: '#2196F3' }
        },
    ...
    function renderNodes(nodesGeojson) {
        ...
        const layer = L.geoJSON(nodesGeojson, {
            ...
            onEachFeature: function (feature, layer) {
                ...
                const imgUrl = feature.properties && (feature.properties.image || feature.properties.pic || feature.properties.thumbnail);
                let imageHtml = '';
                if (imgUrl) {
    ...
    try { 
        ...
        if (cache.isMuseumsCacheValid()) {
            ...
            if (museumsResp.ok) {
                ...
                const fallbackResp = await fetch(`${apiUrl}/demo/museums?src=Q138511481&dst=Q138511511&network=dummy_datasets/rutas_recorridos_2025.geojson`);
                if (fallbackResp.ok) {
    ...
    document.addEventListener('click', (e) => {
        ...
        if (routeSidebar.style.right === '0px' && 
    ...
    searchKgBtn.addEventListener('click', () => {
        kgSearchPanel.style.display = kgSearchPanel.style.display === 'none' ? 'block' : 'none';
    ...
    enhanceKgBtn.addEventListener('click', () => {
        enhanceKgPanel.style.display = enhanceKgPanel.style.display === 'none' ? 'block' : 'none';
        searchPanel.style.display = 'none';
        kgSearchPanel.style.display = 'none';
        analyzeDataPanel.style.display = 'none';
    });
    ...
    analyzeDataBtn.addEventListener('click', () => {
        analyzeDataPanel.style.display = analyzeDataPanel.style.display === 'none' ? 'block' : 'none';
        searchPanel.style.display = 'none';
        kgSearchPanel.style.display = 'none';
        enhanceKgPanel.style.display = 'none';
    });
    ...
    const expertCategorySelection = document.getElementById('expertCategorySelection');
    ...
    ['modPrefHistory', 'modPrefNature', 'modPrefFamily', 'modPrefTime'].forEach(id => {
        const chk = document.getElementById(id);
        if(chk) {
            chk.addEventListener('change', (e) => {
                e.target.parentElement.style.background = e.target.checked ? '#fff3e0' : '#f5f5f5';
                e.target.parentElement.style.borderColor = e.target.checked ? '#FF9800' : '#eee';
            });
        }
    });
    ...
    async function fetchAndDisplayRecommendations(prefs, modalResultsDiv, modalReasoningDiv) {
        try {
            ...
            const bottomReasoningDiv = document.getElementById('expertBottomReasoning');
            ...
            bottomReasoningDiv.innerHTML = data.reasoning.map(r => `• ${r}`).join('<br>');
            
            bottomResultsDiv.innerHTML = ''; // Limpiar resultados anteriores
            
            if (data.recommendations && data.recommendations.length > 0) {
                const recommendedIds = data.recommendations.map(p => p.properties.id || p.properties.qid || p.id);
                
                // Resaltar en mapa
                if (typeof map !== 'undefined') {
                    map.eachLayer((layer) => {
                        if (layer instanceof L.CircleMarker && layer.feature && layer.feature.properties) {
                            const id = layer.feature.properties.id || layer.feature.properties.qid || layer.feature.id;
                            if (recommendedIds.includes(id)) {
                                layer.setStyle({ color: '#E65100', fillColor: '#FF9800', radius: 12, weight: 3, fillOpacity: 1 });
                                if (layer.bringToFront) layer.bringToFront();
                            } else {
                                layer.setStyle({ color: '#2E7D32', fillColor: '#4CAF50', radius: 8, weight: 2, fillOpacity: 0.7 });
                            }
                        }
                    });
                }
                ...
                data.recommendations.forEach((place, i) => {
                    ...
                    card.style.cssText = 'background:#f9f9f9; border-top:4px solid #FF9800; padding:15px; border-radius:8px; display:flex; flex-direction:column; gap:10px; min-width:250px; max-width:280px; box-shadow:0 2px 8px rgba(0,0,0,0.1); flex-shrink:0;';
                ...
                flyBtns.forEach(btn => {
                    btn.addEventListener('click', (e) => {
                        ...
                        if (typeof map !== 'undefined') {
                            ...
                            map.eachLayer((layer) => {
                                if (layer instanceof L.CircleMarker && layer.feature && layer.feature.properties) {
                                    const layerId = layer.feature.properties.id || layer.feature.properties.qid || layer.feature.id;
                                    if (layerId === id) {
                                        layer.openPopup();
                                    }
                                }
                ...
                bottomResultsDiv.innerHTML = '<div style="color:#FF9800; padding:15px; background:#fff3e0; border-radius:8px; width:100%; text-align:center;">No se encontraron recomendaciones exactas cerca de ti.</div>';
            }
